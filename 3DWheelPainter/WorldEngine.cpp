...
namespace Engine {
	...
	float WorldEngine::IntersectWithCylinderForWheelGroup(const glm::mat4& model, /*const glm::vec3& cylinderCenter, */float cylinderWidth, float cylinderRadius, const glm::vec3& ray_dir, const glm::vec3& cameraPosition)//仅限于Z轴方向上的轮子pick
	{
	...
	void WorldEngine::ActivateWheel(WheelSide side, int id, int outputId)//设定要激活轮子的位置,和轮子被点击时输出的ID
	{
		if (side == WheelSide::A)
		{
