...
namespace Wheel {
	...
	const float WheelFlangeTread::PIR = 3.1415926;
	const float WheelFlangeTread::FITBEGINNODE = 31.0f;//从32开始拟合，32之前的不变,一直拟合到surveyTreadBegin
	const float WheelFlangeTread::FITENDNODE = 108.0f;//从surveyTreadEnd拟合到108.0f结束
	const float WheelFlangeTread::SURVEYTREADBEGIN = 50;
	const float WheelFlangeTread::SURVEYTREADEND = 80;
	const float WheelFlangeTread::STANDARDTREADRADIUS = 393.6;  //后面要改！
	const float WheelFlangeTread::STANDARDWIDTH = 135;
	const float WheelFlangeTread::CIRCUMFERENCE = 600;
	...
	//}
	Range WheelFlangeTread::GetTreadRange() const
	{
		return Range(this->surveyTreadRange);
	}
	...
	{
		this->surveyTreadRange.SetRange(surveyTread.firstKey(), surveyTread.lastKey());

		std::cout << "TreadStart:" << this->surveyTreadRange.GetStart() << " SurveyTreadEnd:" << this->surveyTreadRange.GetEnd() << std::endl;
	...
	void WheelFlangeTread::InitSurveyFlange(QVector<QVector2D>& surveyFlange)
	{
		...
		for (int i = 0; i != surveyFlangeSize; ++i)
		{
			...
			if (surveyFlange[i].x() >= surveyTreadRange.GetStart())//截断超范围数据
			{
				id = i;
				break;
			}
	...
	void WheelFlangeTread::InitStandardOutsideTread(std::vector<QVector2D> &standardOutsideTread)//读取外踏面数据
	{
		...
		float realTread;
		file >> realTread;
		float dy = STANDARDTREADRADIUS - realTread;//半径校准
		while (file >> x >> y)
		{
			if (x <= this->surveyTreadRange.GetEnd())//截断超范围数据
			{
				continue;
			}
	...
	const std::map<float, std::vector<Polar>>& WheelFlangeTread::get_flange_tread()
	{
		return flangeTread;
	}
	...
	void WheelFlangeTread::InitFlangeTread(/*std::map<float, std::vector<Polar>> &flangeTread,*/const QVector<QVector2D>& surveyFlange, const QMap<float, QVector<float> >& surveyTread, const std::vector<QVector2D> & standardOutsideTread)
	{
		...
		for (int i = 0; i != surveyFlangeCircleSize; ++i)//将激光数据导入轮缘踏面数据集_SurveyFlangeTread
		{
			...
			if (surveyFlange[i].x() > FITBEGINNODE)
			{
				if (first)
				{
					first = false;
			...
			this->flangeTread.insert({ surveyFlange[i].x(),circlePoint }); //surveyFlange[i].x<=50mm
		}
		...
		for (map_iterator = begin; map_iterator != end; ++map_iterator) {
			...
			this->flangeTread.insert({ map_iterator.key(),circlePoint });
		}
		...
		std::vector<Polar>& leftTread = this->flangeTread[surveyTreadMinX];//SurveyTread最左边的一圈点
		for (int i = 0; i != section; ++i)//拟合
		{
			...
			{
				this->flangeTread[surveyFlange[j].x()][i].r = surveyFlange[j].y() + ddr*(k++);
			}
		}
		...
		for (int i = 0; i != standardOutsideTreadCircleSize; ++i)
		{
			...
			if (standardOutsideTread[i].x() < FITENDNODE)
			{
			...
			this->flangeTread.insert({ standardOutsideTread[i].x(),circlePoint }); //surveyFlange[i].x<=50mm
		}

		//拟合右边
		nearY = standardOutsideTread[0].y();
		std::vector<Polar>& rightTread = this->flangeTread[surveyTreadMaxX];//SurveyTread最右边的一圈点
		for (int i = 0; i != section; ++i)//拟合
		{
			...
			{
				this->flangeTread[standardOutsideTread[j].x()][i].r = standardOutsideTread[j].y() + ddr*(k++);
			}
		}
	}
	std::vector<glm::vec2> WheelFlangeTread::GetStandardTreadMsg() const
	{
		std::vector<glm::vec2> standardTreadMsg;
		StandardTreadMsgMake stmMake;

		auto begin = this->flangeTread.cbegin();
		auto end = this->flangeTread.cend();
		for (std::map<float, std::vector<Polar>>::const_iterator i = begin; i != end; ++i)//每一圈
		{
			standardTreadMsg.push_back(glm::vec2(i->first, stmMake.GetHeight(i->first)));
		}
		return standardTreadMsg;
	}
	WheelSpreadMsg WheelFlangeTread::LoadSpreadPoint()
	{
		std::vector<Vertex> vertices;
		std::vector<unsigned int> indices;
		std::vector<glm::vec2> standardTreadMsg;

		int flangeTreadCircleSize = this->flangeTread.size();
		...
		auto begin = this->flangeTread.begin();
		auto end = this->flangeTread.end();
		std::vector<Vertex> ciclePoint;
		Vertex vertex;
		for (std::map<float, std::vector<Polar>>::iterator i = begin; i != end; ++i)//每一圈
		{
			...
			for (int j = 0; j != section; ++j)  //每一圈上的点
			{
				...
				vertex.Position = glm::vec3(startX + j*unitDis, i->second[j].r - STANDARDTREADRADIUS, i->first);
		...
		smg.wheelSpreadParm = WheelSpreadParm(0.05, unitDis, Range(startX, endX), surveyTreadRange, Range(minY, maxY), standardTreadMsg);
		return smg;
	}
	...
	//}
	OpenglObj::OpenglPointMsg WheelFlangeTread::LoadFlangeTreadPoint(/*std::map<float, std::vector<Polar>> &flangeTread,*/ /*std::vector<QVector2D> &floatPlant*/)
	{
		std::vector<Vertex> vertices;
		std::vector<unsigned int> indices;
	...
	std::vector<glm::vec2> InitFloatPlateData(QString filePath)
	{
		...
		float realTread;
		file >> realTread;
		float dy = WheelFlangeTread::STANDARDTREADRADIUS - realTread;
		while (file >> x >> y)
		{
	...
	OpenglObj::OpenglPointMsg LoadFloatPlantPoint(const std::vector<glm::vec2>& floatPlant)
	{
		...
		float unitRadian = 2 * WheelFlangeTread::PIR / (float)section;
		std::vector<Vertex> ciclePoint;
		Vertex vertex;
