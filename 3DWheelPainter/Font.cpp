...
namespace Font {
	...
	OpenglObj::OpenglPointMsg LoadFontPoint(const std::string& text, const glm::vec3& position, float size)
	{
		...
		{
			Vertex up_left;
			up_left.Position = glm::vec3(origin_position.x + i*kFontWidth*size, origin_position.y, origin_position.z);
			
			Vertex up_right;
			up_right.Position = glm::vec3(origin_position.x + (i+1)*kFontWidth*size, origin_position.y, origin_position.z);

			Vertex down_left;
			down_left.Position = glm::vec3(origin_position.x + i*kFontWidth*size, origin_position.y-kFontHigth*size, origin_position.z);

			Vertex down_right;
			down_right.Position = glm::vec3(origin_position.x + (i+1)*kFontWidth*size, origin_position.y-kFontHigth*size, origin_position.z);
