...
namespace OpenglObj {
	...
	OpenglPointMsg LoadCircle(glm::vec3 circle_center, float radius, int section, glm::vec3 normal)
	{
		if (section < 4)
		{
		...
		glm::vec3 rotate_axis = glm::normalize(glm::cross(origin_normal, circle_center));
		float rotate_radian = glm::angle(origin_normal, circle_center);
		glm::mat4 rotate = glm::rotate(glm::mat4(1.0f), rotate_radian, rotate_axis);//¶ÔÆëfront
